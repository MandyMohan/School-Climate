...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("DD" & s & ":DH" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("DD" & s & ":DH" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("N1").Value = "Prevalence to Teasing and Bulling"
  Sheets("Mean Scores").Range("N" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("DD" & g & ":DH" & g))
      w = Application.WorksheetFunction.Count(Sheets("TransformData").Range("DD" & g & ":DH" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A14").Value = "Prevalence to Teasing and Bullying"
    Sheets("Score Results").Range("B14").Value = sch
