...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("AE" & s & ":AI" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("AE" & s & ":AI" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("G1").Value = "Cultural Competence"
  Sheets("Mean Scores").Range("G" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("AE" & g & ":AI" & g))
      r = Application.WorksheetFunction.Count(Sheets("TransformData").Range("AE" & g & ":AI" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A7").Value = "Cultural Competence"
    Sheets("Score Results").Range("B7").Value = sch
