...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("AO" & s & ":AT" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("AO" & s & ":AT" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("L1").Value = "Leadership"
  Sheets("Mean Scores").Range("L" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("AO" & g & ":AT" & g))
      w = Application.WorksheetFunction.Count(Sheets("TransformData").Range("AO" & g & ":AT" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A12").Value = "Leadership"
    Sheets("Score Results").Range("B12").Value = sch
