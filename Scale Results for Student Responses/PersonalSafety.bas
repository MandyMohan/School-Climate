...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("AU" & s & ":AX" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("AU" & s & ":AX" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("M1").Value = "Personal Safety"
  Sheets("Mean Scores").Range("M" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("AU" & g & ":AX" & g))
      w = Application.WorksheetFunction.Count(Sheets("TransformData").Range("AU" & g & ":AX" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A13").Value = "Personal Safety"
    Sheets("Score Results").Range("B13").Value = sch
