...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("AY" & s & ":BB" & s & ",DI" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("AY" & s & ":BB" & s & ",DI" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("Q1").Value = "Physical Climate"
  Sheets("Mean Scores").Range("Q" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("AY" & g & ":BB" & g & ",DI" & g))
      w = Application.WorksheetFunction.Count(Sheets("TransformData").Range("AY" & g & ":BB" & g & ",DI" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A17").Value = "Physical Climate"
    Sheets("Score Results").Range("B17").Value = sch
