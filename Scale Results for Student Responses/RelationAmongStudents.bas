Attribute VB_Name = "RelationAmongStudents"
Sub Relationship()
Dim L As Long
Dim L1 As Long
Dim m As Long
...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("R" & s & ":U" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("R" & s & ":U" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("F1").Value = "Relationship Among Students"
  Sheets("Mean Scores").Range("F" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("R" & g & ":U" & g))
      r = Application.WorksheetFunction.Count(Sheets("TransformData").Range("R" & g & ":U" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A6").Value = "Relationships Among Students"
    Sheets("Score Results").Range("B6").Value = sch
