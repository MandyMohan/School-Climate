Attribute VB_Name = "RiskyBehaviour"
Sub Behaviour()
Dim L As Long
Dim L1 As Long
Dim m As Long
...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("BQ" & s & ":BS" & s & ",BX" & s & ":BZ" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("BQ" & s & ":BS" & s & ",BX" & s & ":BZ" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("S1").Value = "Risky Behaviour"
  Sheets("Mean Scores").Range("S" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("BQ" & g & ":BS" & g & ",BX" & g & ":BZ" & g))
      w = Application.WorksheetFunction.Count(Sheets("TransformData").Range("BQ" & g & ":BS" & g & ",BX" & g & ":BZ" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A19").Value = "Risky Behaviour"
    Sheets("Score Results").Range("B19").Value = sch
