Attribute VB_Name = "SchoolDiscipline"
Sub Discipline()
Dim L As Long
Dim L1 As Long
Dim m As Long
...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("AJ" & s & ":AN" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("AJ" & s & ":AN" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("K1").Value = "School Discipline"
  Sheets("Mean Scores").Range("K" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("AJ" & g & ":AN" & g))
      w = Application.WorksheetFunction.Count(Sheets("TransformData").Range("AJ" & g & ":AN" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A11").Value = "School Disciplinary Structure"
    Sheets("Score Results").Range("B11").Value = sch
