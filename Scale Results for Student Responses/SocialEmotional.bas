...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("CL" & s & ":CP" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("CL" & s & ":CP" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("H1").Value = "Social Emotional Learning"
  Sheets("Mean Scores").Range("H" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("CL" & g & ":CP" & g))
      r = Application.WorksheetFunction.Count(Sheets("TransformData").Range("CL" & g & ":CP" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A8").Value = "Social Emotional Learning"
    Sheets("Score Results").Range("B8").Value = sch
