Attribute VB_Name = "StudentEngagement1"
Sub Affective()
Dim L As Long
Dim L1 As Long
Dim m As Long
...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("I" & s & ":K" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("I" & s & ":K" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("C1").Value = "Student Engagement:Affective Engagement"
  Sheets("Mean Scores").Range("C" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("I" & g & ":K" & g))
      r = Application.WorksheetFunction.Count(Sheets("TransformData").Range("I" & g & ":K" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A3").Value = "Student Engagement: Affective Engagement"
    Sheets("Score Results").Range("B3").Value = sch
