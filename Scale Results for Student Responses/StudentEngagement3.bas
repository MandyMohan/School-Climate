Attribute VB_Name = "StudentEngagement3"
Sub Behavioural()
Dim L As Long
Dim L1 As Long
Dim m As Long
...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("O" & s & ":Q" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("O" & s & ":Q" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("E1").Value = "Student Engagement:Behavioural Engagement"
  Sheets("Mean Scores").Range("E" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("O" & g & ":Q" & g))
      r = Application.WorksheetFunction.Count(Sheets("TransformData").Range("O" & g & ":Q" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A5").Value = "Student Engagement: Behavioural Engagement"
    Sheets("Score Results").Range("B5").Value = sch
