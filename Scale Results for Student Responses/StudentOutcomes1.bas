Attribute VB_Name = "StudentOutcomes1"
Sub Suspension()
Dim L As Long
Dim L1 As Long
Dim m As Long
...
For v = 1 To L1
  thisarray(v) = Sheets("Sheet1").Range("BC" & s)
  Sheets("Mean Scores").Range("T1").Value = "Student Outcomes: Suspension"
  Sheets("Mean Scores").Range("T" & s).Value = thisarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Sheets("TransformData").Range("BC" & g)
      g = g + 1
    Next m
    ...
    Sheets("Score Results").Range("A20").Value = "Suspension"
    Sheets("Score Results").Range("B20").Value = sch
