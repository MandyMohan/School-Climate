Attribute VB_Name = "StudentOutcomes2"
Sub Absent()
Dim L As Long
Dim L1 As Long
Dim m As Long
...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("BD" & s & ":BM" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("BD" & s & ":BM" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("U1").Value = "Student Outcomes: Absenteeism"
  Sheets("Mean Scores").Range("U" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("BD" & g & ":BM" & g))
      w = Application.WorksheetFunction.Count(Sheets("TransformData").Range("BD" & g & ":BM" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A21").Value = "Absenteeism"
    Sheets("Score Results").Range("B21").Value = sch
