Attribute VB_Name = "StudentSupport2"
Sub Willing()
Dim L As Long
Dim L1 As Long
Dim m As Long
...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("AA" & s & ":AD" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("AA" & s & ":AD" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("B1").Value = "Student Support:Willingness to Ask for Help"
  Sheets("Mean Scores").Range("B" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("AA" & g & ":AD" & g))
      r = Application.WorksheetFunction.Count(Sheets("TransformData").Range("AA" & g & ":AD" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A2").Value = "Student Support: Willingness to seek help"
    Sheets("Score Results").Range("B2").Value = sch
