Attribute VB_Name = "TeacherExpectation1"
Sub Expectation()
Dim L As Long
Dim L1 As Long
Dim m As Long
...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("CC" & s & ":CF" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("CC" & s & ":CF" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("I1").Value = "Teachers Expectation: Academic Expectations"
  Sheets("Mean Scores").Range("I" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("CC" & g & ":CF" & g))
      w = Application.WorksheetFunction.Count(Sheets("TransformData").Range("CC" & g & ":CF" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A9").Value = "Teachers Expectation"
    Sheets("Score Results").Range("B9").Value = sch
