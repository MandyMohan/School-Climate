Attribute VB_Name = "Victimization1"
Sub BullyVict()
Dim L As Long
Dim L1 As Long
Dim m As Long
...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("CT" & s & ":CY" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("CT" & s & ":CY" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("O1").Value = "Victimisation:Bullying Experiences"
  Sheets("Mean Scores").Range("O" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("CT" & g & ":CY" & g))
      w = Application.WorksheetFunction.Count(Sheets("TransformData").Range("CT" & g & ":CY" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A15").Value = "Bullying Victimisation"
    Sheets("Score Results").Range("B15").Value = sch
