Attribute VB_Name = "Victimization2"
Sub Victim()
Dim L As Long
Dim L1 As Long
Dim m As Long
...
For v = 1 To L1
  thisarray(v) = Application.WorksheetFunction.Sum(Sheets("Sheet1").Range("CZ" & s & ":DC" & s))
  w = Application.WorksheetFunction.Count(Sheets("Sheet1").Range("CZ" & s & ":DC" & s))
  ...
  Else
    avarray(v) = ""
  End If
  Sheets("Mean Scores").Range("P1").Value = "Victimisation:Victim Experiences"
  Sheets("Mean Scores").Range("P" & s).Value = avarray(v)
  s = s + 1
Next v
...
For Each x In ActiveWorkbook.Sheets("Sheet1").Range("DL2:DL" & lst)
    ...
    For m = 1 To L
      sarray(m) = Application.WorksheetFunction.Sum(Sheets("TransformData").Range("CZ" & g & ":DC" & g))
      w = Application.WorksheetFunction.Count(Sheets("TransformData").Range("CZ" & g & ":DC" & g))
      If sarray(m) <> 0 Then
    ...
    Sheets("Score Results").Range("A16").Value = "Victim Experiences"
    Sheets("Score Results").Range("B16").Value = sch
