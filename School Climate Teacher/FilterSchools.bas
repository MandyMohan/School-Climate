...
Set rng = .Range("A1:BF" & last)
End With

Workbk.Sheets(sht).Range("B1:B" & last).AdvancedFilter Action:=xlFilterCopy, CopyToRange:=Range("BJ1"), Unique:=True

' Loop through unique values in column
For Each x In Workbk.Sheets(sht).Range([BJ2], Cells(Rows.Count, "BJ").End(xlUp))
...
'Save new workbook
newBook.SaveAs _
        Filename:="C:\Users\" & Environ("username") & "\Documents\School Climate\" & x.Value & " School Climate Teachers Report 2022.xlsx"
